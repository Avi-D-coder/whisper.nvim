...
local function clear_debug_file(config)
  if config and config.debug and config.debug_file then
    ...
    if file then
      file:write(string.format('=== whisper.nvim debug session started at %s ===\n', os.date('%Y-%m-%d %H:%M:%S')))
      file:close()
    end
  end
end
...
M.start_recording = function(config)
  ...
  if config.enable_streaming then
    ...
    -- Set up manual trigger keybind
    local trigger_key = config.manual_trigger_key or '<Space>'
    local trigger_desc = 'Insert transcribed text'
    debug_log(config, 'Setting up ' .. trigger_key .. ' keybind for buffer ' .. buf)

    -- Store the trigger key for later cleanup
    state.set_trigger_key(trigger_key)

    -- Save original keymaps before overriding
    local original_keymaps = {}
    local existing_n = vim.fn.maparg(trigger_key, 'n', false, true)
    local existing_i = vim.fn.maparg(trigger_key, 'i', false, true)
    if existing_n and existing_n.buffer == buf then
      original_keymaps.n = existing_n
    end
    if existing_i and existing_i.buffer == buf then
      original_keymaps.i = existing_i
    end
    state.set_original_keymaps(original_keymaps)

    -- Set our keymaps
    vim.keymap.set('n', trigger_key, M.manual_trigger_insertion, { buffer = buf, desc = trigger_desc })
    vim.keymap.set('i', trigger_key, function()
      M.manual_trigger_insertion()
      return trigger_key -- Still insert the original key in insert mode
    end, { buffer = buf, expr = true, desc = trigger_desc })
    debug_log(config, trigger_key .. ' keybind configured')
  else
  ...
  if config.notifications then
    local trigger_key = config.manual_trigger_key or '<Space>'
    -- Format the key nicely for display (remove < and >)
    local display_key = trigger_key:gsub('[<>]', '')
    vim.notify(
      'Recording... (' .. display_key .. '=insert, ' .. config.keybind .. '=stop)',
      vim.log.levels.INFO
    )
  end
end
